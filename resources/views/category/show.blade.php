...
@section('content')
    ...
    <div class="row">
        ...
        <div class="col-12 col-xl-9 data-mdb-infinite-direction" style="height: 43em; overflow-y: scroll">
                
            <h3 class="h4 mt-4 mb-2 p-0 rounded">| {{ $actCategory->name }}</h3>
            
            @if($actCategory->posts->isNotEmpty())
            
                <table class="postsTable table table-striped table-bordered table-responsive">
                    
                    <tbody>
                        
                        @foreach($actCategory->posts as $post)
                            <tr>
                            ...
                            <tr>
                                ...
                                <td class="comments">
                                    <div class="text-right"><a href="{{ url('post/' . $post->id) }}" class="mr-2">Cmmts.</a>{{ DB::table('comment')->where('idPost', $post->id)->count('id') }}</div>
                                </td>
                                <td class="datePost">
